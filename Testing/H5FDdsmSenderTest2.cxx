...
void particle_write(
    ParticleBuffer_t *buf, const char *filename,
    hsize_t len,  hsize_t start, hsize_t total, H5FDdsmBuffer *dsmBuffer)
{
  hid_t      file_id, group_id, dataset_id, xfer_plist_id;
  hid_t      file_space_id, mem_space_id;
  hid_t      acc_plist_id;
  hid_t      rank = 1;
  hsize_t    count[2]    = {total, 1};
  hsize_t    offset[2]   = {start, 0};
  hsize_t    localdim[2] = {len, 1};
  herr_t     status = 0;
    ...
    MPI_Comm dcomm, H5FDdsmBuffer *dsmBuffer)
{
  ...
  for (i=0; i<N; i++) {
    doublearray[i] = (H5FDdsmFloat64) (i + start + step_increment);
  }
  ...
  particle_write(&WriteBuffer, filename, N, start, total, dsmBuffer);
  MPI_Barrier(dcomm);
      ...
      if (MBytes < remoteMB) {
        ...
        for (int avg = 0; avg < AVERAGE; avg++) {
          if (type == 0) {
            ...
            totaltime += TestParticleWrite(fullname, numParticles, dsmManager->GetUpdatePiece(),
                dsmManager->GetUpdateNumPieces(), comm, dsmManager->GetDSMHandle());
          }
          else if (type==1) {
            totaltime += TestParticleWrite(hdffile.c_str(), numParticles, dsmManager->GetUpdatePiece(),
                dsmManager->GetUpdateNumPieces(), comm, NULL);
          }
        }
