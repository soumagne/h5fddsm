...
int main(int argc, char *argv[])
{
  ...
  while (dsmManager->GetDsmIsConnected()) {
    if (dsmManager->WaitForUpdateReady() > 0) {
      ...
      status = TestParticleRead(fullname, dsmManager->GetUpdatePiece(), nremoteprocs*numParticles, comm, dsmManager->GetDSMHandle());
      if (status == H5FD_DSM_FAIL) dsmManager->H5Dump();
      // Sync here
      MPI_Barrier(comm);
