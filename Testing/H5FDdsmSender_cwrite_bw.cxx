  ...
  for (int type = 0; type < TYPES; type++) {
    ...
    if (MBytes < remoteMB) {
      ...
      for (int skip = 0; skip < SKIP; skip++) {
        if (type == 0) {
          TestParticleWrite(fullname, numParticles, dsmManager->GetUpdatePiece(), dsmManager->GetUpdateNumPieces(), comm, dsmManager->GetDSMHandle());
        }
        else if (type == 1) {
          TestParticleWrite(hdffile.c_str(), numParticles, dsmManager->GetUpdatePiece(), dsmManager->GetUpdateNumPieces(), comm, NULL);
        }
      }
