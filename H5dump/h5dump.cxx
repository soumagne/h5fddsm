...
table_list_add(hid_t oid, unsigned long file_no)
{
    ...
    if(table_list.nused == table_list.nalloc) {
        ...
        if(NULL == (tmp_ptr = HDrealloc(table_list.tables, table_list.nalloc * sizeof(table_list.tables[0]))))
            return -1;
        table_list.tables = (h5dump_table_t*)tmp_ptr;
    } /* end if */
    ...
    if(sset) {
        if(!sset->start || !sset->stride || !sset->count || !sset->block) {
            ...
            if(!sset->start)
                ...
                sset->start = (hsize_t*)calloc(ndims, sizeof(hsize_t));

            if(!sset->stride) {
                unsigned int i;

                sset->stride = (hsize_t*)calloc(ndims, sizeof(hsize_t));
            ...
            if (!sset->count) {
                ...
                sset->count = (hsize_t*)calloc(ndims, sizeof(hsize_t));
            ...
            if (!sset->block) {
                ...
                sset->block = (hsize_t*)calloc(ndims, sizeof(hsize_t));
 ...
 *        Jerome Soumagne
 *        December 2009
 *        modify main function to become H5dump function with an fapl argument
 *
 *-------------------------------------------------------------------------
 */
int
...
main(int argc, const char *argv[])
{
    ...
    output_stream.str("");
    output_stream.clear();
    d_status = H5dump(argc, argv, NULL);
