 ...
 *      Jerome Soumagne, Tuesday, December 1, 2009
 *   Added DSM support
 *-------------------------------------------------------------------------
 */
static hid_t
h5tools_get_fapl(hid_t fapl, const char *driver, unsigned *drivernum, void *dsmBuffer)
{
    ...
    if (!strcmp(driver, drivernames[SEC2_IDX])) {
        ...
        if(!h5tools_mpi_init_g)
            MPI_Initialized(&h5tools_mpi_init_g);
        if(h5tools_mpi_init_g) {
            if(H5Pset_fapl_dsm(new_fapl, H5FD_DSM_INCREMENT, dsmBuffer) < 0)
                goto error;
                           ...
                           unsigned int row_dim/* index of row_counter dimension */)
{
    ...
    for (; hyperslab_count > 0; temp_start[row_dim] += temp_stride[row_dim], hyperslab_count--) {
        ...
        if (sm_nelmts > 0) {
            ...
            if((sm_buf = (unsigned char*)malloc((size_t) sm_nelmts * p_type_nbytes)) == NULL)
